...
*/
		...
		*/
		...
		do_action: function() {
			//console.log('acf.do_action(%o)', arguments);
		...
		*/
		...
		apply_filters: function() {
			//console.log('acf.apply_filters(%o)', arguments);
			...
			if (args.search) {
				...
				$el2.find('[id*="' + args.search + '"]').each(function() {
					$(this).attr(
						...
						$(this)
							.attr('id')
							.replace(args.search, args.replace)
					);
				});
	...
	*/
	...
	acf.add_action('sortstart', function($item, $placeholder) {
		...
		if ($item.is('tr')) {
			...
			$placeholder.html(
				'<td style="height:' + $item.height() + 'px; padding:0;" colspan="' + $item.children('td').length + '"></td>'
			);
		}
	});
...
*/
		...
		render: function(json) {
			...
			$('.acf-postbox.acf-hidden')
				...
				.each(function() {
					$(this)
						.addClass('acf-hidden-by-postbox')
						.prop('disabled', true);
				});
		},
		...
		*/
		...
		_change_parent: function(e) {
			...
			// update & fetch
			this.update('page_type', page_type)
				.update('page_parent', page_parent)
				.fetch();
		},
	...
	acf.fields.google_map = acf.field.extend({
		...
		*/
		...
		initialize: function() {
			...
			// map
			var map_args = acf.apply_filters(
				'google_map_args',
				{
					scrollwheel: false,
			...
			// marker
			var marker_args = acf.apply_filters(
		...
		_add_select_frame_events: function(frame, settings) {
			...
			frame.on('content:activate:browse', function() {
				...
				if (settings.mime_types) {
					// explode
					var extra_types = settings.mime_types
						.split(' ')
						.join('')
						.split('.')
						.join('')
...
(function($) {
	acf.fields.relationship = acf.field.extend({
		...
		focus: function() {
			...
			(this.$choices = this.$el.find('.choices')), (this.$values = this.$el.find('.values'));
...
*/
		...
		fetch: function() {
			...
			this.$choices
				.find('ul:last')
				.append('<p><i class="acf-loading"></i> ' + acf._e('relationship', 'loading') + '</p>');
...
(function($) {
	// globals
	var _select2, _select23, _select24;
	...
	_select2 = acf.select2 = acf.model.extend({
		...
		*/
		...
		do_function: function(name, args) {
			...
			if (typeof this[model] === 'undefined' || typeof this[model][name] === 'undefined') return false;
		...
		init: function($select, args, $field) {
			...
			if (args.multiple) {
				...
				select2_args.formatSelection = function(object, $div) {
					// vars
					var html =
						'<input type="hidden" class="select2-search-choice-hidden" name="' +
						name +
						'" value="' +
						object.id +
						'"' +
						($input.prop('disabled') ? 'disabled="disabled"' : '') +
						' />';
...
(function($, undefined) {
	...
	acf.fields.accordion = acf.field.extend({
		...
		initialize: function() {
			...
			// add icon
			$label.prepend(
				'<i class="acf-accordion-icon dashicons dashicons-arrow-' + (settings.open ? 'down' : 'right') + '"></i>'
			);
